...
Đây là lý do bạn có thể gặp lỗi khi dùng IDL từ chương trình cũ với Anchor SDK mới (hoặc ngược lại).
...
Trong một program Rust mới (không phải staking-app), dùng `invoke` để gọi staking-app với raw instruction data:
