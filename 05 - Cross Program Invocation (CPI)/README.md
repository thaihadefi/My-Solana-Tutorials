...
Đến cuối phần này, bạn sẽ có thể tương tác với các chương trình bên ngoài và xây dựng các giao thức có thể kết hợp với phần còn lại của hệ sinh thái Solana.
...
2. Nó đóng gói những thứ này vào một `CpiContext`, tùy chọn bao gồm các hạt giống người ký (signer seeds) nếu chương trình gọi đang sử dụng một PDA có quyền quản trị.
3. Nó gọi hàm hỗ trợ CPI của chương trình mục tiêu. Anchor sẽ tự mã hóa và gửi CPI instruction bằng Solana runtime.
...
Bây giờ Ứng dụng Ngân hàng của chúng ta đã sẵn sàng cho CPI, hãy triển khai chỉ dẫn `invest` thực tế, cho phép tài khoản quản trị Ngân hàng thực hiện stake hoặc rút SOL vào Ứng dụng Staking bên ngoài.  
...
Dòng code dưới đây là nơi diễn ra tương tác thực tế giữa các chương trình — chỉ với vài dòng mã, chúng ta có thể định tuyến tiền từ Ứng dụng Ngân hàng vào một chiến lược staking tạo ra lợi nhuận một cách an toàn và bảo mật.
...
- Gọi instruction `stake` trong chương trình khác — như thể nó là một phần của chương trình hiện tại  
...
1. **Xây dựng một Chương trình Staking dựa trên Token**
...
- `InvestToken` — cho phép quản trị ngân hàng đầu tư các token SPL đã gửi vào chương trình staking mới của bạn bằng cách sử dụng CPI
