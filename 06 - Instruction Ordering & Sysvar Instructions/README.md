...
Bài học này tập trung vào hai chủ đề liên quan chặt chẽ: **cách Solana thực thi các instruction trong một giao dịch** và **cách chương trình on-chain có thể kiểm tra (introspect) các instruction khác trong cùng giao dịch** thông qua Sysvar Instructions. Ngoài ra, bạn sẽ học các giới hạn runtime quan trọng cùng các kỹ thuật tối ưu hóa bộ nhớ mà bất kỳ nhà phát triển Solana nào cũng cần nắm vững.

---

Kiến thức tiên quyết:

- PDA, ATA, CPI (Phần II - IV)
- Cấu trúc giao dịch và Versioned Transaction (Phần V)
...
- Biết khi nào dùng `invoke_signed` trực tiếp thay vì Anchor CPI wrapper
...
Quy tắc vàng trong smart contract: **kiểm tra trước, thay đổi sau** (checks-effects-interactions pattern). Luôn xác minh tất cả precondition trước khi mutate bất kỳ state nào.
...
Khi chương trình được gọi qua CPI hoặc gộp cùng instruction khác, bạn cần đảm bảo:
...
Nó cho phép chương trình on-chain **đọc tất cả top-level instruction** trong giao dịch hiện tại — bao gồm program ID, danh sách tài khoản, và dữ liệu instruction.
...
// Kiểm tra program_id — phải đến từ chương trình này
...
Anchor sinh discriminator bằng `sha256("global:<instruction_name>")[0..8]`. Bạn tính off-chain rồi hardcode vào chương trình.
...
Dưới đây là ví dụ một chương trình yêu cầu `approve` phải chạy ngay trước `execute`:
...
Bên trong một chương trình, call stack (gọi hàm lồng nhau) giới hạn ở **64 frame**. Đệ quy sâu hoặc chuỗi hàm quá dài sẽ gây lỗi `CallDepthExceeded`.
...
## 7. `invoke_signed` vs Anchor CPI Wrapper

### Vấn đề: CPI depth bị lãng phí

Khi dùng Anchor CPI helper, call stack sâu hơn cần thiết:
...
Mỗi layer wrapper thêm một level trong call stack. Nếu chương trình của bạn đã ở CPI depth 2-3, vài layer nữa có thể vượt giới hạn.

### Giải pháp: Gọi `invoke_signed` trực tiếp

Bỏ qua wrapper, tự xây instruction và gọi `invoke_signed`:
...
**Khi nào dùng:** chương trình đã ở CPI depth sâu, hoặc instruction đơn giản chỉ cần gọi một chương trình khác. **Khi nào KHÔNG dùng:** instruction phức tạp cần nhiều account validation — Anchor wrapper an toàn hơn.
