...
- **CPI (Cross-Program Invocation)** – Cách program gọi program khác
- **Cấu trúc giao dịch** – Các account, signer, instruction  
Nếu bạn cần ôn lại, hãy xem lại các bài học trước trong khóa học này.

---

Kết thúc bài học này, bạn sẽ nắm được:

- Cách Solana thực thi instruction: tuần tự và có tính nguyên tử (atomic)
...
- Khi nào nên sử dụng `invoke_signed` thay vì CPI để kiểm soát số lượng instruction (độ sâu call stack)
...
- Cách kiểm thử và gỡ lỗi các giao dịch phụ thuộc vào thứ tự instruction
...
Ví dụ: Một DEX swap program nên kiểm tra độ trượt giá (slippage) *trước khi* thực hiện swap, chứ không phải sau đó.
...
Các Solana program bị ràng buộc phải chạy nhanh, và để tạo điều kiện cho việc này, ngăn xếp lệnh gọi của chương trình bị giới hạn ở độ sâu tối đa 64 khung (frames).
...
Khi bạn sử dụng `CpiContext` của Anchor, bạn đang xây dựng một lệnh gọi CPI làm tăng độ sâu của call stack:

```rust
// Đây là CPI (làm tăng độ sâu callstack)
...
`invoke_signed` là một hàm cấp thấp hơn có thể hiệu quả hơn cho các thao tác đơn giản, bằng cách sử dụng `invoke_signed`, stack sẽ nông hơn:
...
Sử dụng Bảng Tra cứu Địa chỉ (Address Lookup Tables) từ các giao dịch phiên bản (đã được học trong Bài 05):
...
Khi một tài khoản trở nên rất lớn (thường là sau khi được mở rộng qua nhiều giao dịch), việc tải nó một cách bình thường với `Account<T>` hoặc thậm chí `Box<Account<T>>` có thể gây ra lỗi thiếu bộ nhớ hoặc vi phạm ngăn xếp, vì Anchor cố gắng deserialize toàn bộ tài khoản vào bộ nhớ của chương trình.

Zero-copy giúp tránh điều đó. Thay vì sao chép dữ liệu vào ngăn xếp (stack) hoặc vùng nhớ heap, chương trình của bạn đọc các byte trực tiếp từ bộ đệm bộ nhớ của tài khoản. Về cơ bản, bạn không di dời ngọn núi, bạn làm việc trực tiếp tại nơi nó đứng.
...
Anchor cung cấp một struct Rust nằm trên stack.  
...
Nói chung, một thực hành tốt là cấp phát các tài khoản lớn hơn trên vùng nhớ heap. Mặc dù bạn không thể cấp phát các chương trình theo cách này và các tài khoản heap cần được deserialize (Vì vậy bạn không thể thực hiện điều đó trên các Unchecked Accounts), phương pháp này có thể tiết kiệm một lượng đáng kể không gian ngăn xếp.
...
Mẹo ở đây không phải là giảm độ sâu, mà là giảm **kích thước frame**
...
Cả ba mảng này đều nằm trong cùng một stack frame. Thậm chí nếu chúng được sử dụng tại các thời điểm khác nhau, trình biên dịch vẫn dự trữ không gian cho tất cả chúng **cùng một lúc**. 
...
Đây không trực tiếp là một cách để xử lý stack frame violation, mà giống như việc ngăn chặn `Context` của bạn sử dụng bộ nhớ khổng lồ.
