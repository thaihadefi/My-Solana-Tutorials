...
✅ Đúc (Mint) SPL token đầu tiên ?  
✅ Học cách lấy và tạo ATA bằng client Anchor TS  
✅ Tích hợp ATA vào Bank app để cho phép gửi và rút token  

Sau khi hoàn thành phần này, bạn sẽ có thể tạo, quản lý và tương tác với các tài khoản token SPL như một cách thành thạo — đặt nền tảng cho mọi chức năng liên quan đến chuyển token, phần thưởng hoặc thanh toán.  
Bắt đầu thôi! 💰🚀

### Nhớ lại ví dụ trước: Bank app 🏦

Trong phần này, chúng ta sẽ mở rộng Bank app để hỗ trợ các token SPL. Cụ thể, chúng ta sẽ thêm hai instruction mới:

- `DepositToken` — cho phép người dùng gửi bất kỳ SPL token nào vào ngân hàng
- `WithdrawToken` — cho phép người dùng rút lại chính token mà họ đã gửi trước đó

Bản nâng cấp này biến Bank app của bạn từ một ứng dụng chỉ hỗ trợ SOL thành kho lưu trữ có thể xử lý nhiều loại token — một bước tiến tới gần hơn với DeFi trong thực tế. Hãy cùng xây dựng! 🧱💸
...
✅ Giống như các PDA khác, ATA không có private key và được lấy ra một cách có xác định từ các seed. Đó là điều làm cho các ATA có thể dễ dàng xác định và an toàn.

#### 🤔 Tại sao chúng ta cần ATA?

Trong Solana, người dùng không giữ token trực tiếp trong địa chỉ ví của họ. Thay vào đó, mỗi SPL token (như USDC, wSOL, v.v.) được lưu trữ trong một Tài khoản Token (Token Account) — một tài khoản đặc biệt theo dõi số dư của một loại token cụ thể.
...
Một Tài khoản Token Liên kết (ATA) là một tài khoản token tiêu chuẩn được lấy ra từ một ví và một token mint. Nó đảm bảo:
...
Thật may mắn, việc tạo một SPL token tùy chỉnh trên Solana cực kỳ dễ dàng — CLI `spl-token` xử lý hầu hết các công việc nặng nhọc cho bạn.
...
Bây giờ bạn đã hiểu ATA là gì, tại sao chúng quan trọng và đã mint thành công SPL token đầu tiên của mình bằng CLI, đã đến lúc thực hiện bước tiếp theo — làm việc với ATA bằng lập trình bằng **Anchor TypeScript**.
...
Hàm này tạo instruction để khởi tạo một ATA trên chuỗi (on-chain):
...
Bây giờ là lúc để áp dụng tất cả những gì bạn đã học! Bạn sẽ hoàn thành một loạt các bài tập có hướng dẫn để hoàn thiện Ứng dụng Ngân hàng. Bạn sẽ thêm logic để xử lý việc rút token và hoàn thành quy trình gửi/rút tiền đầy đủ bằng cách sử dụng ATA — giúp ứng dụng của bạn sẵn sàng hoạt động với các SPL token thực tế trên Solana.

🛠️ Nhiệm vụ của bạn: 

1. **Triển khai `token_transfer_from_pda` trong `transfer_helper.rs`**

Hàm này sẽ chuyển bất kỳ SPL token nào (hiện tại là Token V1 cổ điển) từ một PDA (như `BankInfo`) trở lại người dùng.  
Hãy đảm bảo sử dụng `invoke_signed()` (giống như khi chuyển SOL từ PDA), và bao gồm các `signer_seeds` chính xác.

1. **Hoàn thành Lệnh rút Token (WithdrawToken)**

Cho phép người dùng rút SPL token đã gửi của họ từ vault (PDA `BankInfo`) vào tài khoản token của chính họ.

1. **Viết các bài kiểm tra trong `bank-app.ts`**

Và cuối cùng, đừng bao giờ quên viết test! Xác nhận rằng logic rút tiền của bạn hoạt động như mong đợi bằng cách sử dụng bộ công cụ kiểm tra Anchor.

Sau khi giải quyết xong các nhiệm vụ này, Ứng dụng Ngân hàng của bạn sẽ hỗ trợ đầy đủ việc gửi và rút SPL token thông qua ATA.  
