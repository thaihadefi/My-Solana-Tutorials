...
- Biết cách tìm kiếm IDL của một program
- Biết cách từ IDL tạo ra một crate để thực hiện CPI.

## 1. IDL là gì?

Một tệp Interface Description Language (IDL) cho Anchor program cung cấp một file JSON được tiêu chuẩn hóa mô tả các instruction và tài khoản của program. Tệp này đơn giản hóa quá trình tích hợp on-chain program với các ứng dụng client

Nói đơn giản, IDL không phải là program của bạn - nó là một bản mô tả bằng JSON cho các client biết cách tương tác với chương trình của bạn. Hãy nghĩ về nó như là tài liệu API của chương trình, nhưng được cấu trúc dành cho máy tính đọc. 

Bạn sẽ sử dụng IDL này để chạy program sau khi deploy thông qua JS/TS, hoặc để tương tác với các chương trình khác.

Nó bao gồm:

- **Instructions**: Chữ ký hàm, các kiểu tham số và các tài khoản bắt buộc
- **Accounts**: Các định nghĩa struct cho dữ liệu do program quản lý
- **Types**: Các kiểu dữ liệu tùy chỉnh được sử dụng xuyên suốt các instruction
- **Events**: Các cấu trúc dữ liệu nhật ký (log) được emit ra
...
- Phân tích cú pháp mã nguồn Rust của bạn để trích xuất chữ ký instruction, cấu trúc tài khoản và các kiểu dữ liệu.
...
Nếu bạn chạy lệnh `anchor idl init -f <target/idl/program.json> <program-id>`, Anchor sẽ tạo một tài khoản IDL, được sở hữu bởi chính program đó. Lưu ý rằng, đây không phải là mặc định - nghĩa là IDL không tự động được tải lên on-chain nếu bạn chạy lệnh `anchor deploy`.

Thực tế thì tôi không thấy ai sử dụng tính năng này, không ai tải IDL của chương trình lên on-chain cả :D. Trong trường hợp đó, làm sao bạn có thể tìm thấy IDL của một chương trình?
...
- `args`: Các tham số instruction cùng với kiểu dữ liệu
- `returns`: Kiểu trả về (thường là null đối với các chương trình Solana)
- `pda`: Thông tin để derive PDA
...
1. Trong dự án đó, đổi tên `main.rs` thành `lib.rs`, và viết:

```rust
anchor_gen::generate_cpi_crate!("../../path/to-idl/idl.json"); 
```

1. Chạy `cargo build` và bạn sẽ có CPI. Repo `anchor-gen` không tạo ra tệp Rust cho CPI, nó tạo ra CPI **nội bộ** sử dụng proc macro, có thể xem được bằng lệnh `cargo expand`.
2. Sau khi tạo xong crate CPI, bạn thực hiện import nó:
