...
Bây giờ bạn đã quen với việc viết các program cơ bản trên Solana, đã đến lúc tìm hiểu một trong những khái niệm quan trọng nhất trong phát triển Solana — Program Derived Addresses - PDAs. Các account đặc biệt này đóng vai trò cốt lõi để xây dựng các program an toàn, có trạng thái (stateful), có thể lưu trữ dữ liệu người dùng, quản lý kho lưu trữ (vaults), kiểm soát authority, và hơn thế nữa.
...
Sau khi kết thúc phần này, bạn sẽ có thể tự tin tạo và quản lý các PDA trong các program của mình, mở khóa khả năng xây dựng các program mạnh mẽ và phức tạp hơn.  
Bắt đầu thôi! 🧠✨

### Hãy bắt đầu với một ví dụ thực tế: Ứng dụng Ngân hàng 🏦

Để hiểu cách PDA hoạt động trong thực tế, hãy xem xét một program ngân hàng đơn giản trên Solana.  
Trong ứng dụng này:

👤 Người dùng có thể gửi và rút SOL  
🛑 Admin có thể tạm dừng program để dừng mọi hoạt động trong trường hợp khẩn cấp  
💾 Program nên lưu trữ:
...
Program Derived Address (PDA) là một loại Solana account đặc biệt được sở hữu bởi một program, không phải bởi một ví trên đường cong (ví người dùng) có khóa bí mật (private key). Điều này làm cho PDA trở thành xương sống của hầu hết các program Solana — chúng cho phép program của bạn quản lý trạng thái, tài sản và thẩm quyền một cách an toàn mà không phụ thuộc vào các ví sở hữu bên ngoài.

Các PDA:  
🔐 *Được kiểm soát bởi program* — không có private key, chỉ program mới có thể truy cập vào các PDA của nó và không ai có thể giả mạo chữ ký của PDA.  
🧠 *Có tính xác định (Deterministic)* — chúng được tạo bằng các đầu vào cố định (gọi là `seeds`) cộng với ID program.  
✍️ *Có khả năng ký giao dịch* — nhưng chỉ bằng cách sử dụng `invoke_signed()` với các `seeds` của PDA bên trong program.

Trong program ngân hàng của chúng ta, chúng ta sử dụng hai PDA để lưu trữ dữ liệu trong `state.rs`:
...
- `BankInfo` là một PDA lưu trữ trạng thái của program: ai là `authority`, trạng thái của program có phải `is_paused` hay không, và giá trị `bump` của Bank Vault.
...
Để khắc phục điều này, họ thêm một số nhỏ — bump (một số nguyên không dấu 8 bit từ 0–255) — số này được điều chỉnh tự động trong quá trình tạo PDA để đảm bảo một địa chỉ hợp lệ. Anchor tự động xử lý tính toán bump khi bạn khởi tạo PDA. Nhưng nếu program của bạn cần tạo lại hoặc ký thay cho PDA đó, bạn phải lưu trữ bump để có thể tái tạo các hạt giống hoặc địa chỉ chính xác.

👉 Trong ví dụ của chúng ta, chúng ta lưu trữ bump trong `BankInfo` vì program sẽ cần PDA Bank Vault để ký các instruction sau này.

### 2. Khởi tạo một PDA

Bây giờ chúng ta đã hiểu PDA là gì, hãy cùng tìm hiểu cách tạo và khởi tạo một PDA trong Anchor.  
Trong ứng dụng ngân hàng của chúng ta, chúng ta khởi tạo PDA `BankInfo` khi program được thiết lập lần đầu tiên. Đây là đoạn code trong `instructions/initialize.rs`:
...
- *Sở hữu bởi hệ thống*: Tài khoản được tạo với `owner = system_program::ID`, nghĩa là nó được sở hữu bởi System Program, không phải program Anchor của bạn. Điều này nghe có vẻ bất thường lúc đầu, nhưng đó là có chủ đích.
- *Tại sao lại tạo PDA này?*  
Vault này đóng vai trò là nơi giữ tiền tập trung cho toàn bộ hệ thống của bạn. Vì nó là một PDA được lấy ra bằng ID program của bạn và một seeds đã biết, program của bạn vẫn có thể ký thay cho nó và kiểm soát số dư SOL của nó.

**⚠️ Lưu ý quan trọng**: Lý do chúng ta sử dụng một PDA do System Program sở hữu là vì chỉ những tài khoản do System Program sở hữu mới có thể tham gia vào việc chuyển SOL gốc. Khi chuyển SOL bằng lệnh transfer, cả người gửi và người nhận đều phải là các tài khoản do hệ thống sở hữu. Đó là lý do tại sao chúng ta cấu trúc vault theo cách này — do program kiểm soát mà người dùng có thể gửi tiền vào hoặc rút tiền ra. Chúng ta sẽ tìm hiểu sâu hơn về cách thức hoạt động của tính năng này khi chúng ta triển khai logic chuyển SOL thực tế trong phần tiếp theo.
...
Đó là kết thúc cho instruction `Initialize`.
...
Trong hàm này, trước khi cho phép bất kỳ việc chuyển tiền nào, program trước tiên sẽ kiểm tra trạng thái của `BankInfo`:
...
Việc chuyển SOL thực tế được xử lý bằng một hàm hỗ trợ được định nghĩa trong `transfer_helper.rs`:
...
Sau này, khi chúng ta triển khai việc rút tiền, program sẽ cần ký thay cho PDA Bank Vault — và đối với việc đó, chúng ta sẽ sử dụng `invoke_signed()`.
...
Bây giờ bạn đã biết cách tạo, khởi tạo và tương tác với các PDA bên trong program, hãy cùng chuyển sang phía client.  
➡️ Trong phần tiếp theo, chúng ta sẽ tìm hiểu cách lấy các địa chỉ PDA từ client Anchor TypeScript để có thể gọi các lệnh này một cách chính xác từ frontend hoặc các script.

### 3. Lấy PDA phía Client

Để tương tác với program của bạn từ frontend hoặc script (như gọi `initialize` hoặc `deposit`), bạn sẽ cần lấy chính xác các địa chỉ PDA mà program cần. Anchor giúp việc này trở nên dễ dàng ở phía client TypeScript.

Hãy xem cách thực hiện bằng cách sử dụng chính logic mà chúng ta đã sử dụng trong program.  
...
- `SEEDS` là một mảng các byte (Buffer) phải khớp chính xác với những gì program sử dụng.
- `PROGRAM_ID` là ID program đã triển khai của bạn.
...
Lưu ý rằng `userReserve` là một hàm. Điều này cho phép bạn tạo động một PDA duy nhất cho mỗi người dùng dựa trên public key của họ.  
Bằng cách lấy PDA theo cách này, bạn đảm bảo client của mình luôn sử dụng đúng các tài khoản — chính xác như program của bạn mong đợi.
...
1. **Hoàn thành Lệnh rút tiền (Withdraw)**
...
1. **Triển khai Lệnh tạm dừng (Pause)**
...
1. **Đừng quên viết các bài kiểm tra trong `bank-app.ts`**
