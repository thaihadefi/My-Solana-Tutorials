...
Sau khi kết thúc phần này, bạn sẽ có thể tự tin tạo và quản lý các PDA trong các smart contract của mình, mở khóa khả năng xây dựng các smart contract mạnh mẽ và phức tạp hơn.  
Bắt đầu thôi! 🧠✨
...
Program Derived Address (PDA) là một loại Solana account đặc biệt được sở hữu bởi một chương trình, không phải bởi một ví trên đường cong (ví người dùng) có khóa bí mật (private key). Điều này làm cho PDA trở thành xương sống của hầu hết các smart contract Solana — chúng cho phép chương trình của bạn quản lý trạng thái, tài sản và thẩm quyền một cách an toàn mà không phụ thuộc vào các ví sở hữu bên ngoài.

Các PDA:  
🔐 *Được kiểm soát bởi chương trình* — không có private key, chỉ chương trình mới có thể truy cập vào các PDA của nó và không ai có thể giả mạo chữ ký của PDA.  
🧠 *Có tính xác định (Deterministic)* — chúng được tạo bằng các đầu vào cố định (gọi là `seeds`) cộng với ID chương trình.  
✍️ *Có khả năng ký giao dịch* — nhưng chỉ bằng cách sử dụng `invoke_signed()` với các `seeds` của PDA bên trong chương trình.
...
Trong ứng dụng ngân hàng của chúng ta, chúng ta khởi tạo PDA `BankInfo` khi chương trình được thiết lập lần đầu tiên. Đây là đoạn code trong `instructions/initialize.rs`:
